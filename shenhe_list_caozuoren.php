...
$sort    = isset($_REQUEST['sort']) ? $_REQUEST['sort'] : 'DESC'; // 获取前台搜索框中 排序类型，默认为倒序
    //  设置为当前用户的数据
    $where .= " AND caozuoren='".$_SESSION["username"]."' ";
    ...
    // 设置zulin模块，表id 的值是提交过来，有则只查询这些内容
     ...
     if($_REQUEST["zulinid"] !=  "" ){     $where .= " AND zulinid='".$_REQUEST["zulinid"]."' ";
    }
// 根据搜索提交的表单搜索数据
                 ...
                 if($_GET["lianxidianhua"] != null && !"" ==  $_GET["lianxidianhua"] ){ $where.=" AND lianxidianhua LIKE '%".$_GET["lianxidianhua"]."%'"; } 
...
<div class="form-search pa10 bg-warning">
    <form class="form-inline" action="?" size="small" id="formSearch"><!-- form 标签开始 -->
                   ...
                   联系电话： <input type="text" class="form-control" style="" name="lianxidianhua" value="<?php echo $_GET["lianxidianhua"]; ?>"/>
                    
                   租赁类型： <select class="form-control class_zulinleixing5" data-value="<?php echo $_GET["zulinleixing"]; ?>" id="zulinleixing" name="zulinleixing">
<option value="">请选择</option><option value="整租">整租</option>
<option value="分租">分租</option>

</select>
<script>
$(".class_zulinleixing5").val($(".class_zulinleixing5").attr("data-value"))</script>

                    
                   审核： <select class="form-control class_shenhe6" data-value="<?php echo $_GET["shenhe"]; ?>" data-rule-required="true" data-msg-required="请填写审核" id="shenhe" name="shenhe">
<option value="">请选择</option><option value="通过">通过</option>
<option value="未通过">未通过</option>

</select>
<script>
$(".class_shenhe6").val($(".class_shenhe6").attr("data-value"))</script>
                    ...
                    <td width="30" align="center">
                        <label>
                            ...
                            <?php echo $map["zulinleixing"]; ?>                        </td>
                                            <td>
                            <?php  $mapfangjianxinxi6 = db()->find("SELECT fangjianmingcheng,id FROM fangjianxinxi where id='".$map["xuanzefangjian"]."'");  ?><?php echo $mapfangjianxinxi6["fangjianmingcheng"]; ?>                        </td>
                                            <td>
                            ...
                            <?php echo $map["caozuoren"]; ?>                        </td>
                                                            ...
                                                            <td align="center">
                                                                        <a class="btn btn-info btn-xs" href="shenhe_detail.php?id=<?php echo $map["id"]; ?>" title="详情"> 详情 </a>  
 

<a class="btn btn-success btn-xs" href="shenhe_updt.php?id=<?php echo $map["id"]; ?>" title="编辑"> 编辑 </a>  
 

<a class="btn btn-danger btn-xs" href="shenhe.php?a=delete&id=<?php echo $map["id"]; ?>" onclick="return confirm('确定删除？此操作不可恢复')" title="删除"> 删除 </a>  

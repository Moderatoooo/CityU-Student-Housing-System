...
{//类定义开始
    ...
    public static function getMaxSystemUpload()
    {
    ...
    {
        $upload = new static();
        $upload->maxSize = self::getMaxSystemUpload();
        $upload->autoSub = false;
        //$upload->allowExts = [];//explode(',','jpg,png,gif,zip,rar,doc,xls,xlsx,pdf,ppt,pptx,wps,docx,gz,gzip,7z');
        $upload->savePath = DT_PATH.'upload/';
        $upload->saveRule = 'getSaveName';
	...
	/**
	 * @access public
	 * @param string $file 上传的base64 数据
	 * @return bool
	 */
	function UploadBase64Image($base64file){
		//如果不指定保存文件名，则由系统默认
        if(empty($savePath))
            $savePath = $this->savePath;
        // 检查上传目录
		...
		if(substr($base64file , 0,22) == 'data:image/png;base64,'){
			$base64file = substr($base64file , 22);
		}

		$iamge = base64_decode($base64file);
		...
		file_put_contents($filename, $iamge);

		if(!$this->checkSize(filesize($filename))) {
            $this->error = '上传文件大小不符！'.byte_format(filesize($filename));
            return false;
        }
	...
	function saveImageToServer($file)
	{
		$save = get_sava_type();
		$save->open();
		$saveto = $file['savepath'] . $file['savename'];

		$newfile = $save->save($saveto);
		if($newfile === false && $save_type != 'host'){
			$save->close();
			$save = jntoo_drive::create('uploadsave', 'host');
			$newfile = $save->save($saveto);
			$save_type = 'host';
		}
		...
		$data['filemd5'] = $uploadList[0]['hash'];
		$data['save_type'] = $save->save_type;
		$data['filepath'] = $newfile;

		$attach = jntoo_drive::create('attach', 'system');

		$aid = $attach->insert('images' , $data);
		$returndata['aid']		= $aid;
		// 是附件 就记录下来
		$returndata['filepath'] = $save->url . $data['filepath'];
		$returndata['fileext']  = $data['fileext'];
		$returndata['isimage']  = 1;
    ...
    {
        return !($size > $this->maxSize) || (-1 == $this->maxSize) || (0 == $this->maxSize);
    }
